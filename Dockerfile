...
RUN echo "deb http://archive.ubuntu.com/ubuntu quantal main universe" > /etc/apt/sources.list && \
    ...
    add-apt-repository "deb http://archive.ubuntu.com/ubuntu quantal main universe restricted multiverse" && \
    apt-get install -y build-essential mingw-w64-i686-dev binutils-mingw-w64-i686 gcc-mingw-w64-base gcc-mingw-w64-i686 g++-mingw-w64-i686 pkg-config dos2unix wget
...
RUN mkdir -p /usr/i686-w64-mingw32/bin && \
    ...
    ln -s /usr/i686-w64-mingw32/libogg_static.lib /usr/i686-w64-mingw32/lib/ogg_static.lib && \
    ln -s /usr/i686-w64-mingw32/libogg_static_d.lib /usr/i686-w64-mingw32/lib/ogg_static_d.lib && \
    ln -s /usr/i686-w64-mingw32/libvorbis.lib /usr/i686-w64-mingw32/lib/vorbis.lib && \
    ln -s /usr/i686-w64-mingw32/libvorbis_d.lib /usr/i686-w64-mingw32/lib/vorbis_d.lib && \
    ln -s /usr/i686-w64-mingw32/libvorbis_static.lib /usr/i686-w64-mingw32/lib/vorbis_static.lib && \
    ln -s /usr/i686-w64-mingw32/libvorbis_static_d.lib /usr/i686-w64-mingw32/lib/vorbis_static_d.l && \
    ln -s /usr/i686-w64-mingw32/libvorbisfile.lib /usr/i686-w64-mingw32/lib/vorbisfile.lib && \
    ln -s /usr/i686-w64-mingw32/libvorbisfile_d.lib /usr/i686-w64-mingw32/lib/vorbisfile_d.lib && \
    ln -s /usr/i686-w64-mingw32/libvorbisfile_static.lib /usr/i686-w64-mingw32/lib/vorbisfile_static.lib && \
...
# Fix line endings on some shell scripts. Just in case you're using git on windows with autocrlf set to true :)
...
# Set up entrypoint, working directory, default command and mark /uqm as being a volume which needs to be bound later..
WORKDIR /uqm
VOLUME ["/uqm"]
CMD ["uqm"]
